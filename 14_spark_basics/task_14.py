...
def run():
    print("="*65)
    print(f"  TASK 14: {TASK_FOCUS.get('14','Spark Task')}")
    print("="*65)
    print("\n  Running comprehensive Spark simulation (tasks 12-17 combined)...")
    results = hadoop_run()
    (OUT/"spark_task_14_results.json").write_text(
        json.dumps(results, indent=2, default=str))
    print(f"\n  Focus areas covered in this task:")
    print(f"    {TASK_FOCUS.get('14','')}")
    print(f"\n✓ Results: {OUT}/spark_task_14_results.json")
    return results
